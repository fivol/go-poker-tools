...
// https://github.com/mroth/weightedrand/blob/main/weightedrand.go
...
func NewChooser(choices ...Choice) (*Chooser, error) {
	...
	return &Chooser{data: choices, totals: totals, max: runningTotal, rnd: fastrand.RNG{}}, nil
}
...
func (c Chooser) Pick() interface{} {
	// TODO concurrent unsafe method
	r := int(c.rnd.Uint32n(uint32(c.max)) + 1)
